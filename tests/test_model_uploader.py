...
class TestModelUploader(unittest.TestCase):
    ...
    def test_download_latest_team_weights_skip_when_identical(self, mock_download, mock_remote_map, mock_sha):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            res = download_latest_team_weights_if_needed(model_type="cnn", hf_config=dummy_cfg, weights_dir=weights_dir)
            self.assertTrue(res)
    ...
    def test_download_latest_team_weights_wav2vec2(self, mock_download, mock_remote_map, mock_sha):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for fname in ("config.json", "labels.json", "model.safetensors", "preprocessor_config.json"):
                (w2v_dir / fname).touch()

            res = download_latest_team_weights_if_needed(model_type="wav2vec2", hf_config=dummy_cfg, weights_dir=weights_dir)
            self.assertTrue(res)
            mock_download.assert_not_called()
    ...
    def test_upload_weights_cnn_only(self, mock_remote_map, mock_sha, mock_hf_api_class, mock_login):
        ...
        dummy_cfg = HuggingFaceConfig(token="dummy_token_123", repo_id="dummy/repo-id", auto_upload=True)
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            res = upload_weights_to_hf(model_type="cnn", hf_config=dummy_cfg, weights_dir=weights_dir, force_upload=True)
            self.assertTrue(res)
    ...
    def test_upload_weights_cnn_skip_identical(self, mock_remote_map, mock_sha, mock_hf_api_class, mock_login):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            res = upload_weights_to_hf(model_type="cnn", hf_config=dummy_cfg, weights_dir=weights_dir, force_upload=False)
            self.assertTrue(res)
    ...
    @patch("utils.model_uploader.calculate_file_sha256", return_value="dummy_hash_123")
    @patch("utils.model_uploader.get_remote_file_sha256_map", return_value={})
    def test_upload_weights_wav2vec2_essential_files_strict_count(self, mock_remote_map, mock_sha, mock_hf_api_class, mock_login):
        mock_api_instance = MagicMock()
        mock_hf_api_class.return_value = mock_api_instance
        dummy_cfg = HuggingFaceConfig(token="dummy_token_123", repo_id="dummy/repo-id")
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            essential_files = ["config.json", "labels.json", "model.safetensors", "preprocessor_config.json"]
            for fname in essential_files:
                (w2v_dir / fname).write_text("dummy content")

            res = upload_weights_to_hf(model_type="wav2vec2", hf_config=dummy_cfg, weights_dir=weights_dir, force_upload=True)
            self.assertTrue(res)
