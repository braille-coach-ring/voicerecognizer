...
# Example: {"shi": PROJECT_ROOT / "sounds" / "special_shi.wav"}
LABEL_SOUND_FILES: dict[str, Path] = {}
...
def play_guide_wav(label: str) -> None:
    ...
    guide_wav = GUIDE_WAV_ROOT / label / "008.wav"

    if not guide_wav.exists():
        print(f"⚠️ お手本音声が見つかりません: {guide_wav}")
        return
